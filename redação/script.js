...
const config = {
    ...
    // CONFIGURAÇÃO CORS PROXY
    USE_CORS_PROXY: true,
    CORS_PROXY: 'https://api.allorigins.win/raw?url=',
    // Alternativas de proxy (descomente para testar):
...
async function makeRequest(url, method = 'GET', headers = {}, body = null) {
    ...
    // Usa proxy apenas para APIs externas (não Gemini)
    if (config.USE_CORS_PROXY && !url.includes('generativelanguage.googleapis.com')) {
        requestUrl = config.CORS_PROXY + encodeURIComponent(url);
    ...
    try {
        ...
        if (!response.ok) {
            ...
            if (response.status === 403) {
                console.error('❌ Erro 403 - Acesso bloqueado (CORS)');
                throw new Error('Acesso bloqueado. A Sala do Futuro está bloqueando requisições externas.');
            }
        ...
        // Se for erro de CORS, mostra mensagem específica
        if (error.message.includes('CORS') || error.message.includes('403') || error.message.includes('bloqueado')) {
            showNotification('⚠️ Erro de acesso. A Sala do Futuro pode estar bloqueando requisições externas. Tente novamente mais tarde.', 'error', 8000);
        }
...
async function fetchRedacaoContent(taskId, token, roomName) {
    ...
    try {
        ...
        if (error.message.includes('403') || error.message.includes('bloqueado')) {
            showNotification('❌ Token expirado ou acesso bloqueado. Faça login novamente.', 'error');
        }
        throw error;
    }
}
...
async function startRedactionProcess(redacao) {
    ...
    try {
        ...
        showNotification('🎉 Obrigado por usar o sistema Trollchipss!', 'info');
...
async function loginAndFetchRedacoes() {
    ...
    try {
        ...
        // Salva a conta após login bem-sucedido
        if (typeof saveAccount === 'function') {
            saveAccount(raInput.value, senhaInput.value);
        }
...
async function sendRegistrationRequest(loginResponseData) {
    try {
        showNotification('📋 Registrando na plataforma...', 'info');
        ...
        if (!data.auth_token) {
            throw new Error('auth_token não retornado pela API');
        }
...
async function fetchUserRoomsForRedacoes(authToken, userNick) {
    try {
        ...
        if (roomUserData.rooms && roomUserData.rooms.length > 0) {
            ...
            showNotification('🔎 Procurando redações...', 'info');
            ...
            if (currentFetchedRedacoes.length > 0) {
                ...
                if (finalPendingCount === 0 && finalDraftCount > 0) {
                    showNotification('ℹ️ Não há redações pendentes, mas há rascunhos salvos.', 'info');
                } else if (finalPendingCount > 0) {
                    showNotification(`✅ Encontradas ${finalPendingCount} redações pendentes e ${finalDraftCount} em rascunho.`, 'success');
                } else if (finalDraftCount > 0) {
                    showNotification(`ℹ️ Você tem ${finalDraftCount} redações em rascunho.`, 'info');
                }
            } else {
                showNotification('⚠️ Nenhuma redação encontrada. A Sala do Futuro pode estar temporariamente indisponível.', 'warning');
            }
        } else {
            showNotification('⚠️ Nenhuma sala encontrada. O sistema pode estar em manutenção.', 'warning');
        }
    } catch (error) {
        console.error('❌ Erro ao buscar salas:', error);
        showNotification('❌ Erro ao buscar redações. Tente novamente mais tarde.', 'error');
        throw error;
    } finally {
        trava = false;
    }
...
function displayRedacoesInSelectionModal(redacoes) {
    ...
    if (redacoes.length === 0) {
        redacaoListContainer.innerHTML = '<p style="text-align: center; color: var(--text-secondary); padding: 20px;">Nenhuma redação encontrada.</p>';
...
searchRedacaoBtn.addEventListener('click', async () => {
    ...
    if (!raInput.value || !senhaInput.value) {
        showNotification('⚠️ Por favor, preencha o RA e a senha.', 'warning');
        return;
    }
...
selectRedacaoBtn.addEventListener('click', async () => {
    ...
    if (checkedCheckboxes.length > 0) {
        ...
        showNotification('⚠️ Selecione uma redação para continuar.', 'warning');
    }
});
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Log de status do proxy
    if (config.USE_CORS_PROXY) {
        console.log('🔄 Proxy CORS ATIVADO:', config.CORS_PROXY);
        console.log('💡 Se houver erro 403, desative o proxy ou use uma extensão CORS');
    }
});

// === LOG DE DESENVOLVIMENTO ===
console.log('%c🎓 Trollchipss Redação System v4.1', 'color: #4a6bff; font-size: 16px; font-weight: bold;');
...
console.log('%c🌐 Proxy CORS configurado', 'color: #8b5cf6; font-size: 12px;');
console.log('%c💾 Sistema de contas salvas disponível', 'color: #06b6d4; font-size: 12px;');
